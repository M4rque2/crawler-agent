...
Write-Host "[1/3] Dumping UI hierarchy on device (compressed mode): $remotePath"
$dumpSucceeded = $false
for ($attempt = 1; $attempt -le $RetryCount; $attempt++) {
    ...
    $dumpOut = & adb @adbBase shell uiautomator dump --compressed $remotePath 2>&1
...
if (-not $dumpSucceeded) {
    throw "uiautomator compressed dump failed after $RetryCount attempts. No remote XML found at $remotePath."
}
