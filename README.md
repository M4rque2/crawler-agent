...
- **Action expectation and mismatch detection.** After each action, the agent should state the expected observable change in the next state (for example, page transition, popup disappearance, scroll displacement, or focus change). If the observed result does not match the expected result, the system should immediately raise attention and enter recovery mode. This is conceptually related to ReAct and Reflexion, but implemented inside one agent loop rather than split across separate executor and reflector agents.
